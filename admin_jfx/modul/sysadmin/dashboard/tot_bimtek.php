 ...
 if($_SESSION['level']=="")
  ...
  if( isset( $_REQUEST['aksi'] )){
    ...
    switch($aksi){
      ...
      break;
      case 'hapus':
        include '../../aksi/sysadmin/masterdat/delete_pbj.php';
        break;
      // case 'cetak':
...
echo '
  ...
  width="100%">
      ...
      $sql = mysqli_query($koneksi, "SELECT  user.* , petugas_pbj.*
        ...
        where level = 'pelaksana' and status_user = 'Active'
        ");
      if(mysqli_num_rows($sql) > 0){
        ...
        echo '
                ...
                <tr>
                 <td>'.$no.'</td>
                 <td><a href="admin?hlm=profile&id_pbj='.$row['nip'].'">'.$row['nama'].'</a></td>
                  <td>'.$row['nip'].' </td>
                 <td>'.$row['lok_pelatihan'].' </td>
                   <td>'.$row['instansi'].' </td>
                   <td>'.$row['lok_bekerja'].' </td>
                   <td>'.$row['jabatan'].' </td>
                     <td>'.$row['thn_pelatihan'].' </td>
                     <td>'.$row['kabupaten'].' </td>
                       <td>'.$row['provinsi'].' </td>
